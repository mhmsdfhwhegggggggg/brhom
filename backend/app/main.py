...
from app.models import (
    ...
    SubscriptionRequestCreate, SubscriptionRequestUpdate, SiteSettingsUpdate
)
...
# ==================== PUBLIC: SUBSCRIPTION & SETTINGS ====================

@app.post("/api/register")
async def register_subscription(data: SubscriptionRequestCreate):
    with get_db() as conn:
        conn.execute(
            """INSERT INTO subscription_requests (name, country, phone, account_wallet, plan_name, notes)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (data.name, data.country, data.phone, data.account_wallet, data.plan_name, data.notes)
        )
        return {"message": "Subscription request submitted successfully. Our team will contact you soon."}


@app.get("/api/settings/{key}")
async def get_setting(key: str):
    with get_db() as conn:
        setting = conn.execute("SELECT value FROM site_settings WHERE key = ?", (key,)).fetchone()
        if not setting:
            return {"value": None}
...
async def get_subscriptions(admin: dict = Depends(get_admin_user)):
    with get_db() as conn:
        subs = conn.execute("SELECT * FROM subscription_requests ORDER BY created_at DESC").fetchall()
        return [dict(s) for s in subs]


@app.put("/api/admin/subscriptions/{sub_id}")
async def update_subscription_status(sub_id: int, data: SubscriptionRequestUpdate, admin: dict = Depends(get_admin_user)):
    with get_db() as conn:
        sub = conn.execute("SELECT * FROM subscription_requests WHERE id = ?", (sub_id,)).fetchone()
        if not sub:
            raise HTTPException(status_code=404, detail="Request not found")
        
        conn.execute("UPDATE subscription_requests SET status = ? WHERE id = ?", (data.status, sub_id))
        
        # If approved, create a client account automatically
        if data.status == "approved" and sub["status"] != "approved":
            access_code = generate_access_code()
            raw_password = generate_password()
            password_hash = hash_password(raw_password)
            
            # Extract capital from plan name if possible or use defaults
            # For simplicity, we create with 0 investment and let admin edit
            conn.execute(
                """INSERT INTO clients (name, phone, access_code, password_hash, subscription_plan, notes)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (sub["name"], sub["phone"], access_code, password_hash, sub["plan_name"], f"Approved Request #{sub_id}. Wallet: {sub['account_wallet']}")
            )
            return {
                "message": "Subscription approved and client account created",
                "access_code": access_code,
                "password": raw_password
            }
            
        return {"message": f"Subscription status updated to {data.status}"}


# ==================== ADMIN: SITE SETTINGS ====================

@app.put("/api/admin/settings/{key}")
async def update_setting(key: str, data: SiteSettingsUpdate, admin: dict = Depends(get_admin_user)):
    with get_db() as conn:
        conn.execute(
